...
#Q1.4 - Change the ggplot object sepal_plot so that color represents the Species.
...
#Q1.5 - Change the plot created in the last question again to add the size attribute so that 
...
ggplot(diamonds, aes(x = clarity, y = carat, col = price)) + geom_jitter()
...
ggplot(mtcars, aes(x = mpg, fill = factor(am))) + geom_histogram(position = "fill", binwidth = 1) 
...
#Q5.3 - Add a facet_wrap layer to the histogram plotted in Q5.2 to plot separate histograms
...
# represents the quality of cutting of the diamonds. Plot separate histograms of 
# prices for each type of cut on the same plot in multiple windows.
# Use binwidth = 100.
...
ggplot(ukacc,aes(x = ukacc$date, y = ukacc$drivers)) + geom_line()
ggplot(ukacc,aes(x = ukacc$date, y = ukacc$drivers))+ geom_point() 
ggplot(ukacc,aes(x = ukacc$date, y = ukacc$drivers))+ geom_smooth
ggplot(ukacc,aes(x = ukacc$date, y = ukacc$drivers)) + geom_line() + geom_smooth(span = 0.1) 
