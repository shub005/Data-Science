...
setwd("C:/Users/Ranjita/Desktop/Data Analytics_IITM/R")
# Reading The file
bollywood <- read.csv("bollywood.csv")
# Viewing the File bollywood
View(bollywood)
...
# Getting the structure of the the data set bollywood
str(bollywood)

# You can change the attribute 'Movie' from factor to character type using the given command
# Changing attributes from factor to Character
...
# Store the movie name in variable named top_2_movie
  ...
  sec <- bollywood[-c(which.max(bollywood$Tcollection)),]
  top_2_movie <- sec[which.max(bolly2$Tcollection),]
	top_2_movie
...
#You can write a command to find the maximum value of Ocollection, Wcollection, Fwcollecion and Tcollection using sapply
	max(sapply(bollywood$Ocollection, FUN = summary),na.rm=TRUE)
	max(sapply(bollywood$Wcollection, FUN = summary),na.rm=TRUE)
	max(sapply(bollywood$Fwcollection, FUN = summary),na.rm=TRUE)
	max(sapply(bollywood$Tcollection, FUN = summary),na.rm=TRUE)
